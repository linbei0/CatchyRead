import type { PlaybackProgressMode, PlaybackStatus, SmartScriptSegment } from '@/lib/shared/types';
...
function mapStatusLabel(status: PlaybackStatus, progressMode: PlaybackProgressMode): string {
  switch (status) {
    case 'preparing':
      return '准备中';
    case 'playing':
      return progressMode === 'media-time' ? '播放中' : '播放中';
    case 'paused':
      return '已暂停';
    case 'error':
      return '出错了';
...
export function buildPlaybackViewState(args: {
  ...
  return {
    ...
    currentSummary: currentSegment
      ? trimSummary(currentSegment.spokenText, 30)
      : '选好模式后开始收听。',
